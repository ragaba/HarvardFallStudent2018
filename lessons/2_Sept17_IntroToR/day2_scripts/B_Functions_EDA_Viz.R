...
# Oversample
s <- sample(1:nrow(screenTime), 25, prob = ifelse(screenTime$duration > mean(screenTime$duration), 0.9, 0.01))
...
# Base plots
barplot(characterTally[1:5], main='Force Awakens: Character Scene Tally')
...
ggplot(screenTime, aes(colour=screenTime$character)) + # data frame then aesthetics
  geom_segment(aes(x=screenTime$start, xend=screenTime$end, y=screenTime$character, yend=screenTime$character), 
